...
\newcommand{\titleLineOne}{Implementation of a Simplistic Interface for}
\newcommand{\titleLineTwo}{Big Data Workload Scheduler in Kubernetes}
\newcommand{\titleLineThree}{}
\newcommand{\documentdate}{March 29, 2022}
\newcommand{\studentname}{Lukas Schwerdtfeger}
\newcommand{\abstracttextde}{Das Datenaufkommen hat sich in den letzten Jahren vervielfältigt  und wird sich voraussichtlich weiter beschleunigen. Die Verarbeitung von Big Data erfordert hochentwickelte Systeme, die mit dem Datenfluss Schritt halten können. Spark und Flink sind oft genutzte Big-Data-Frameworks, die die Entwicklung von verteilten Datenflussanwendungen unterstützen. Spark- und Flink-Anwendungen werden in der Regel auf einem gemeinsam genutzten Cluster eingesetzt, der aus preiswerter Standardhardware besteht. Bisher wurde eine statische Verteilung von Cluster-Ressourcen verwendet, um langlaufende Batch-Anwendungen voneinander zu trennen, die aber mit potenziell ungenutzter Hardware einhergeht. Cluster-Ressourcenmanager wie YARN, Mesos oder Kubernetes ermöglichen eine feinere Verteilung gemeinsam genutzter Cluster-Ressourcen unter Verwendung von sogenannten Containern, so dass Anwendungen dynamischer skaliert werden können, wenn Ressourcen verfügbar sind. Eine feine Aufteilung schafft viele Möglichkeiten, Resourcenverteilungen für Batch-Anwendungen zu planen. Die Entwicklung eines neuen Planungsalgorithmus erfordert ein tiefes Verständnis der inneren Funktionsweise des Clusters. Die Suche nach einer optimalen Resourcenverteilung hat erhebliche Auswirkungen auf die Gesamtressourcennutzung des Clusters und die Laufzeit der Jobs. Kubernetes ist ein Ressourcenmanager, der sich heute als Industriestandard durchgesetzt hat. Seine Schnittstelle unterscheidet sich erheblich von anderen Ressourcenmanagern wie YARN. Planungsalgorithmen, die für YARN entwickelt wurden, können nicht ohne wesentliche Änderungen auf Kubernetes angewendet werden. Diese Arbeit wird eine einfache Schnittstelle zur Steuerung des Scheduling von Batch-Anwendungen auf Kubernetes entwickelt. Die Schnittstelle erweitert Kubernetes um TestBeds, die Slots in einem Cluster reservieren und sie einem externen Scheduler anbieten. Der externe Scheduler übermittelt sein Scheduling, dass die Jobs in die Warteschlange stellt und sie in der Reihenfolge ihrer zugewiesenen Slots ausführt. Die Schnittstelle wird durch die Implementierung eines Profiling-Schedulers mit Hilfe mehrerer Testbeds evaluiert.}
\newcommand{\abstracttext}{Data generation has exploded over the last few years and is not expected to slow down. Big Data processing requires sophisticated systems that can keep up with the data flow. Spark and Flink are popular Big Data frameworks that aid the development of distributed dataflow applications. Spark and Flink applications are commonly deployed on a shared cluster composed of cheap commodity hardware. Previously, static partitioning of cluster resources has been used to isolate long-running batch applications from one another, which comes with potentially unused hardware. Cluster resource managers like YARN, Mesos, and Kubernetes allow a more fine-grain partitioning of shared cluster resources, using containers, allowing applications to scale more dynamically if resources are available. A fine-granular partitioning creates many possible schedulings of batch applications. Developing a new scheduling algorithm requires a deep understanding of the cluster's inner workings. Finding optimal scheduling will significantly impact the cluster's overall resource utilization and runtime of jobs. Kubernetes is a resource manager that has become the defacto industry standard. Its interface differs considerably from other resource managers like YARN. Scheduling algorithms that have been developed for YARN can not be applied to Kubernetes without substantial changes. This work offers a simplistic interface to control the scheduling of batch applications on Kubernetes. The interface extends Kubernetes with Testbeds reserving slots in a cluster and offering them to an external scheduler. The external scheduler submits its scheduling, which will enqueue jobs and run them in order inside their dedicated slots. The interface is evaluated by implementing a profiling scheduler with the help of multiple Testbeds.}
...
\input{generated/Appendix/Repository.tex}
